...
export class AuthController {
  ...
  private cookieOptions() { const production = process.env.NODE_ENV === 'production'; return { httpOnly: true, secure: production, sameSite: production ? 'none' as const : 'lax' as const, path: '/api/v1/auth' }; }
  private setCookie(response: Response, token: string) { response.cookie(COOKIE, token, { ...this.cookieOptions(), maxAge: 30 * 86_400_000 }); }
}
