...
export class ImportsService implements OnModuleInit,OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    const interrupted=await this.prisma.importJob.findMany({where:{scope:{equals:['LIVE']},status:{in:['QUEUED','CONNECTING','DOWNLOADING','PARSING','STAGING','APPLYING']},OR:[{leaseExpiresAt:null},{leaseExpiresAt:{lt:new Date()}}]},orderBy:{createdAt:'asc'},take:2,select:{id:true,sourceId:true,status:true,createdAt:true}});
    ...
    for (const job of completed) await this.prisma.importJob.updateMany({ where: { sourceId: job.sourceId, status: 'PREVIEW', scope:{equals:['LIVE']}, createdAt: { lt: job.createdAt } }, data: { status: 'CANCELLED', errorCode: 'IMPORT_SUPERSEDED', errorDetail: 'IMPORT_SUPERSEDED', finishedAt: new Date() } });
  ...
  async start(sourceId: string, dto: StartImportDto) {
    if (dto.scope.some(kind => kind !== 'LIVE')) throw new BadRequestException('ONLY_LIVE_IMPORTS_SUPPORTED');
    ...
    const current = await this.prisma.importJob.findFirst({ where: { sourceId, scope: { equals: ['LIVE'] }, status: { in: ['QUEUED', 'CONNECTING', 'DOWNLOADING', 'PARSING', 'STAGING', 'PREVIEW', 'APPLYING'] } }, orderBy: { createdAt: 'desc' } });
    if (current) {
    ...
    const job = await this.prisma.importJob.create({ data: { sourceId, scope: ['LIVE'] } }); if(this.mode()!=='enqueue')setImmediate(() => void this.execute(job.id)); return job;
  }
  ...
  async run(jobId: string) {
    ...
    if (job.scope.length !== 1 || job.scope[0] !== 'LIVE') throw new BadRequestException('LEGACY_IMPORT_PAUSED');
    try {
  ...
  private async dispatchAvailable(){const jobs=await this.prisma.importJob.findMany({where:{scope:{equals:['LIVE']},status:{in:['QUEUED','CONNECTING','DOWNLOADING','PARSING','STAGING','APPLYING']},OR:[{leaseExpiresAt:null},{leaseExpiresAt:{lt:new Date()}}]},orderBy:{createdAt:'asc'},take:2,select:{id:true,sourceId:true,status:true,createdAt:true}});for(const job of jobs)if(job.status==='APPLYING')void this.executeApply(job.id,job.sourceId,job.createdAt);else void this.execute(job.id);}
  ...
  private async requirePreview(id: string) { const job = await this.prisma.importJob.findUnique({ where: { id } }); if (!job) throw new NotFoundException('IMPORT_NOT_FOUND'); if (job.scope.length !== 1 || job.scope[0] !== 'LIVE') throw new BadRequestException('LEGACY_IMPORT_PAUSED'); if (job.status !== 'PREVIEW') throw new BadRequestException('IMPORT_PREVIEW_REQUIRED'); return job; } private error(value: unknown) { return (value instanceof Error ? value.message : 'IMPORT_FAILED').replace(/https?:\/\/\S+/g, '[REDACTED_URL]').slice(0, 200); } private slug(value: string) { return normalize(value).replace(/ /g, '-').slice(0, 80) || 'item'; }
