...
export class SourcesService {
 ...
 async channels(id:string,query:{page:number;pageSize:number;search?:string;group?:string}){
  ...
  const [items,total,groups]=await this.prisma.$transaction([
   ...
   const ranked=items.map(item=>{const health=item.channel?.playbackVariants.map(value=>value.health).find(Boolean);return{...item,channel:item.channel?{...item.channel,playbackVariants:undefined}:item.channel,availability:health??{status:'UNKNOWN',latencyMs:null,circuitOpenUntil:null,lastSuccessAt:null,lastFailureAt:null}};}).sort((a,b)=>this.healthRank(a.availability.status)-this.healthRank(b.availability.status));
   return{items:ranked,total,page,pageSize,pages:Math.max(1,Math.ceil(total/pageSize)),groups:groups.map(value=>({name:value.groupName??'Sans catégorie',value:value.groupName??'',count:typeof value._count==='object'&&value._count?value._count.id??0:0}))};
 }
