...
import { providerGenres } from '../imports/content-classifier';
...
export class PortalClient{
 ...
 async *catalogueBatches(source:PortalSource,secret:SourceCredentials,onProgress?:PortalProgress):AsyncGenerator<NormalizedSourceItem[]>{
  ...
  for(const definition of definitions){
   ...
   if(!yielded&&orderedError&&definition.type==='itv')throw orderedError;
  }
 }
 ...
 private item(raw:Record<string,unknown>,kind:'LIVE'|'MOVIE'|'SERIES',genres:Map<string,string>):NormalizedSourceItem|null{const remoteId=String(raw.id??raw.ch_id??raw.movie_id??raw.series_id??''),displayName=String(raw.name??raw.title??'').trim(),command=String(raw.cmd??raw.url??'').trim();if(!remoteId||!displayName)return null;const portalType=kind==='LIVE'?'itv':kind==='MOVIE'?'vod':'series',streamRef=command?`portal:${portalType}:${Buffer.from(command).toString('base64url')}`:undefined,groupId=String(raw.tv_genre_id??raw.category_id??raw.genre_id??''),groupName=genres.get(groupId)??String(raw.category_name??'Sans catégorie'),screens=Array.isArray(raw.screenshots)?raw.screenshots[0]:undefined,logoUrl=String(raw.logo??raw.logo_30??raw.screenshot_uri??raw.cover??raw.cover_big??raw.movie_image??screens??'')||undefined;return{kind,remoteId,fingerprint:createHash('sha256').update(`${kind}|${displayName.toLowerCase()}|${remoteId}`).digest('hex'),displayName,groupName,streamRef,logoUrl,classificationConfidence:1,classificationReason:[`provider:portal:${portalType}`],genres:providerGenres(groupName,raw.genre,raw.description),raw:{year:raw.year,rating:raw.rating,portalType,playbackResolvable:Boolean(command)}}}
