...
export function SecurePlayer({ targetType, targetId, onUnavailable }: { targetType: TargetType; targetId: string; onUnavailable?: () => void }) {
  const video = useRef<HTMLVideoElement>(null);
  const sourceFailures = useRef(0);
  const [status, setStatus] = useState('Préparation du lecteur…');
  ...
  useEffect(() => {
    ...
    const saveProgress = () => {
      ...
      if (!media || targetType === 'CHANNEL' || !Number.isFinite(media.currentTime)) return;
      void fetch(`${api}/library/progress`, {
        method: 'POST',
        headers: { authorization, 'content-type': 'application/json' },
        body: JSON.stringify({
          mediaTitleId: targetType === 'MEDIA' ? targetId : undefined,
          episodeId: targetType === 'EPISODE' ? targetId : undefined,
          positionSec: Math.max(0, Math.floor(media.currentTime)),
          durationSec: Number.isFinite(media.duration) ? Math.max(1, Math.floor(media.duration)) : undefined,
          completed: Number.isFinite(media.duration) && media.duration > 0 && media.currentTime / media.duration > 0.92,
          deviceId,
        }),
      }).catch(() => undefined);
    };
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) {
          ...
          if (!cancelled) {
            setFailed(true);
            setStatus(playbackError(response.status, body.message));
            if ([400, 404, 502].includes(response.status)) fallback(true);
          }
          return;
        }
        const value = await response.json() as { sessionId: string; manifestUrl: string; streamType?: StreamType };
        ...
        if (value.streamType === 'mpegts') {
          ...
          if (media.canPlayType('application/vnd.apple.mpegurl')) {
            ...
            cleanup = () => resetMedia(media, playing, waiting);
            await media.play();
          } else {
            ...
            cleanup = () => { media.removeEventListener('playing', playing); media.removeEventListener('waiting', waiting); created.off(Hls.Events.ERROR, onError); created.destroy(); };
          ...
          cleanup = () => { media.removeEventListener('playing', playing); media.removeEventListener('waiting', waiting); return created.destroy(); };
        ...
        if (targetType !== 'CHANNEL') progressTimer = setInterval(saveProgress, 15_000);
      } catch {
        if (!cancelled) { setFailed(true); setStatus('Impossible de démarrer ce flux pour le moment.'); fallback(); }
      }
    })();
  ...
  }, [targetId, targetType, retry, onUnavailable]);

  return <div className="player-shell"><div className="player-stage"><video ref={video} controls autoPlay playsInline preload="metadata" controlsList="nodownload" />{status && <div className={`player-status ${failed ? 'error' : ''}`}><span className="player-spinner" aria-hidden="true" /><p aria-live="polite">{status}</p>{failed && <button className="button" onClick={() => { setFailed(false); setStatus('Préparation du lecteur…'); setRetry((value) => value + 1); }}>Réessayer</button>}</div>}</div></div>;
}
