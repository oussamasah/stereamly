...
const initial = { name: '', type: 'M3U' as SourceType, baseUrl: '', allowedHosts: '', username: '', password: '', macAddress: '', serialNumber: '', deviceId: '', deviceId2: '', signature: '', userAgent: '', epgUrl: '', regionCode: '', priority: 100, refreshIntervalMin: 360, maxConcurrentStreams: '', syncLive: true, syncMovies: false, syncSeries: false, syncEpg: false, preferHls: true };
...
export function SourceManager() {
  ...
  return <div className="source-layout">
    ...
    <section className="card"><div className="section-copy"><h2>{editingId ? 'Modifier la source' : 'Ajouter une source'}</h2><Link href="./help#sources">Besoin d’aide ? Lire le guide</Link></div><form onSubmit={submit}>
      ...
      <fieldset><legend>Contenus à importer</legend>{(['syncLive'] as const).map(key => <label className="checkbox" key={key}><input type="checkbox" checked={form[key]}  onChange={event => field(key, event.target.checked)}/>{key.replace('sync', '')}</label>)}<p className="field-hint">Les imports sont limités aux chaînes en direct. Les réglages historiques VOD et EPG sont conservés pour la migration.</p></fieldset>
