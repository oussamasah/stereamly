...
function ContentRail({title,items,locale}:{title:string;items:Content[];locale:Locale}){return <section className="content-rail"><h2>{title}</h2><div className="rail-track">{items.map(item=>{const artwork=image(item,item.type?'POSTER':'LOGO')??image(item,'BACKDROP')??item.logoUrl;return <Link className={`rail-card ${item.type?'portrait':'channel'}`} href={href(item,locale)} key={item.id}><div className="rail-art">{artwork?<img src={artwork} alt=""/>:<span>{name(item,locale)}</span>}{!item.type&&<span className="live-pill"><i/> LIVE</span>}<span className="play-overlay">▶</span></div><strong>{name(item,locale)}</strong><small>{item.type==='MOVIE'?'Film':item.type==='SERIES'?'Série':item.category?.names?.[locale]??'En direct'}</small></Link>})}</div></section>}
